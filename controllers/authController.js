...
// Imports des modèles (gardez uniquement ceux que vous utilisez réellement dans ce fichier)
// IMPORTANT: Assurez-vous que ces chemins sont corrects par rapport à votre structure de projet
const Donation = require('../models/Donation');
const Event = require('../models/Event');
const Transaction = require('../models/Transaction');
// Si vous avez un modèle Donor séparé et que vous l'utilisez ici, décommentez:
...
const register = async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, message: error.message || "Une erreur inconnue est survenue lors de l'inscription." });
    }
};
...
const login = async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, message: error.message || "Une erreur est survenue lors de la connexion." });
    }
};
...
const getProfile = async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, message: error.message || "Une erreur est survenue lors de la récupération du profil." });
    }
};
...
const updateProfile = async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, message: error.message || "Une erreur est survenue lors de la mise à jour du profil." });
    }
};
...
const validateMember = async (req, res) => {
    try {
        ...
        if (user.role !== 'member') {
            return res.status(400).json({ message: 'Seuls les utilisateurs ayant le rôle "member" peuvent être validés de cette manière.' });
        }
        ...
        res.json({
            ...
            user: { id: user._id, nom: user.nom, prenom: user.prenom, email: user.email, role: user.role, estValide: user.estValide, isSuperAdmin: user.isSuperAdmin, isAdmin: user.isAdmin }
        });
    } catch (error) {
        console.error('Erreur lors de la validation du membre:', error);
        res.status(400).json({ success: false, message: error.message || "Une erreur est survenue lors de la validation du membre." });
    }
};
...
const getAdminStats = async (req, res) => {
    try {
        ...
        const members = await User.countDocuments({ role: "member", estValide: true }); // Membres validés

        // Agrégation pour le total des dons
        const totalDonationResult = await Donation.aggregate([{ $group: { _id: null, total: { $sum: "$amount" } } }]);
        ...
        const totalIncomeTransactions = totalIncomeTransactionsResult[0]?.total || 0;
        ...
        const totalExpenseTransactions = totalExpenseTransactionsResult[0]?.total || 0;
...
const getUnvalidatedMembers = async (req, res) => {
    try {
        ...
        const unvalidatedMembers = await User.find({ role: 'member', estValide: false }).select('-password');
        res.status(200).json({ success: true, message: 'Membres en attente de validation récupérés avec succès.', data: unvalidatedMembers });
    } catch (error) {
        console.error('Erreur serveur lors de la récupération des membres non validés :', error);
        res.status(500).json({ message: 'Une erreur serveur est survenue lors de la récupération des membres.' });
    }
};
...
const rejectMember = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: 'Demande de membre rejetée avec succès. Utilisateur supprimé.' });
    } catch (error) {
        console.error('Erreur serveur lors du rejet du membre :', error);
        res.status(500).json({ message: 'Une erreur serveur est survenue lors du rejet du membre.' });
    }
};
